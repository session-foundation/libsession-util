...
/// - `stream_encrypted` -- [in] how the file that was uploaded is encrypted, which the url carries
///   as a `d` fragment so that a recipient decrypts it the right way.  This describes the *file*,
///   not the server, so it belongs to whoever did the encrypting: pass true for anything uploaded
///   through `Network::upload_file`, which always encrypts with the stream scheme.  False is for a
///   caller that encrypted the bytes itself with the legacy scheme before uploading them.
///
/// Outputs:

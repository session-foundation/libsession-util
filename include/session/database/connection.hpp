...
struct DeleteResult {
    bool success;
    int return_code;
    size_t count; // Number of rows that were deleted
};
...
struct Connection {
    ...
    AddResult add_pro_revocations(
            std::span<const pro_backend::ProRevocationItem> revocations) noexcept;

    /// API: database/delete_pro_revocations
    ///
    /// Delete a list of Session Pro revocations from the database associated with this connection.
    /// Revocations are deleted by matching the item's `gen_index_hash`. This function is
    /// transactional, on failure changes to the database are rolled back.
    ///
    /// Inputs:
    /// - `revocations` -- The list of revocations to delete
    ///
    /// Outputs:
    /// - `success` -- True if the delete was successful, false otherwise.
    /// - `return_code` -- The SQLite3 error code that caused the error if `success` was false
    /// - `count` -- Number of rows that were deleted in this operation
    DeleteResult delete_pro_revocations(
            std::span<const pro_backend::ProRevocationItem> revocations) noexcept;
    ...
    std::vector<pro_backend::ProRevocationItem> get_pro_revocations();
};
}  // namespace session::database

...
std::vector<std::pair<std::string, int64_t>> Contacts::last_deleted_contacts() const {
    ...
    if (auto info = data["d"].dict()) {
        ...
        for (const auto& [key, value] : *info)
            if (auto* exp_scalar = std::get_if<session::config::scalar>(&value))
                if (auto* exp = std::get_if<int64_t>(exp_scalar))
                    ret.emplace_back(key, *exp);
    }
...
LIBSESSION_C_API bool last_deleted_contacts(const config_object* conf, const contacts_deleted_contact** contacts, size_t* contacts_len) {
    try {
        auto cpp_contacts = unbox<Contacts>(conf)->last_deleted_contacts();
        ...
        auto* ret = (contacts_deleted_contact*)malloc(cpp_contacts.size() * sizeof(contacts_deleted_contact));

        for (size_t i = 0; i < cpp_contacts.size(); ++i) {
            std::memcpy(ret[i].session_id, cpp_contacts[i].first.data(), 67);
            ret[i].session_id[66] = '\0';  // Ensure null termination
            ret[i].deleted = cpp_contacts[i].second;
        }

        *contacts_len = cpp_contacts.size();
        return true;
    } catch (...) {
        return false;
    }

...
namespace {
    ...
    DecryptedResponse decrypt_v4_response(const session::onionreq::ResponseParser& parser, const std::string& response) {
    ...
    DecryptedResponse decrypt_onion_response(
        ...
        if (!onionreq::ResponseParser::response_long_enough(builder.enc_type, encrypted_response.size()))
            throw std::runtime_error{"Response is too short to be an onion request response: " + encrypted_response};
...
void OnionRequestRouter::_finish_setup() {
    ...
    for (auto& [category, queue] : _request_queues) {
        if (!queue.is_empty()) {
            ...
            log::debug(cat, "[OnionRouter]: Processing {} requests queued during initialization for category '{}'.", pending.size(), to_string(category));
...
void OnionRequestRouter::_send_request_internal(Request request, network_response_callback_t callback) {
    auto initiating_req_category = (_config.single_path_mode ? RequestCategory::standard : request.category);

    if (!_ready) {
        log::debug(cat, "[OnionRouter Request {}]: Router not ready, queueing request.", request.request_id);
        
        try {
            _request_queues.at(initiating_req_category).add(std::move(request), std::move(callback));
        } catch (const std::exception& e) {
            log::critical(cat, "[OnionRouter]: No request queue for category '{}', request {} is being dropped.", to_string(initiating_req_category, _config.single_path_mode), request.request_id);
    ...
    log::trace(cat, "[OnionRouter Request {}]: Received request for category '{}', searching for a path.",
        request.request_id, to_string(initiating_req_category, _config.single_path_mode));
    OnionPath* path = _find_valid_path(request);

    if (path) {
        log::debug(cat, "[OnionRouter Request {}]: Found valid path {}, sending.", request.request_id, path->id);
        _send_on_path(*path, std::move(request), std::move(callback));
        return;
    }
     
    // No valid path, queue the request an build a path
    log::debug(cat, "[OnionRouter Request {}]: No path available, queueing request.", request.request_id);
    ...
    const auto current = _paths.count(initiating_req_category) ? _paths.at(initiating_req_category).size() : 0;
    ...
    if (should_build) {
        log::info(cat, "[OnionRouter Request {}]: Path count for '{}' is insufficient, building new path.",
            initiating_req_id, to_string(initiating_req_category, _config.single_path_mode));
        
        _build_path(initiating_req_category, initiating_req_id, {});
    }
}

void OnionRequestRouter::_build_path(RequestCategory category, std::optional<std::string> initiating_req_id, const std::vector<service_node>& nodes_to_exclude_) {
    ...
    log::info(cat, "[OnionRouter Request {} Path {}]: Starting build for {} path.", req_id_log, path_id, to_string(category, _config.single_path_mode));

    // If we were misconfigured to have a `path_length` of `0` then just fail all requests
    if (_config.path_length == 0) {
        log::error(cat, "[OnionRouter Request {} Path {}]: Cannot build path, path_size is configured to 0.", req_id_log, path_id);
        
        auto queue_it = _request_queues.find(category);
        if (queue_it == _request_queues.end()) {
            log::critical(cat, "[OnionRouter]: No request queue for category '{}'.", to_string(category, _config.single_path_mode));
            return;
        }
...
void OnionRequestRouter::_on_guard_connectivity_response(const std::string& path_id, RequestCategory category, std::optional<std::string> initiating_req_id, bool success) {
    ...
    if (pending_it == _pending_paths.end()) {
        log::warning(cat, "[OnionRouter Request {} Path {}]: Received connection callback for a path that is no longer pending, ignoring.", req_id_log, path_id);
        return;
    }
    ...
    if (!pending_requests.empty()) {
        ...
        log::debug(cat, "[OnionRouter Request {} Path {}]: Processing {} queued requests.", req_id_log, path_id, pending_requests.size());
        ...
        // Put any un-sendable requests back into the front of the queue (or fail in `single_path_mode`)
        if (!requeue.empty()) {
            if (_config.single_path_mode) {
                log::warning(cat, "[OnionRouter Path {}]: {} requests could not be sent on the single available path, failing them.", path_id, requeue.size());
                for (const auto& [req, cb] : requeue)
                    cb(false, false, -1, {content_type_plain_text}, "Request destination conflicts with the only available path in single_path_mode");

                return;
            }

            log::debug(cat, "[OnionRouter Path {}]: Unable to process {} queued requests, requing them.", path_id, requeue.size());
            ...
            if (_in_progress_path_builds[category] == 0) {
                log::info(cat, "[OnionRouter]: Building additional {} path for remaining requests.", to_string(category, _config.single_path_mode));
...
OnionPath* OnionRequestRouter::_find_valid_path(const Request& request) {
    ...
    for (OnionPath& path : candidate_paths) {
        ...
        if (target_node) {
            ...
            if (conflict && _config.single_path_mode)
                log::warning(cat, "[OnionRouter Request {}]: Path destination conflicts with the only available path, but single_path_mode is enabled, proceeding.", request.request_id);
            else if (conflict)
                continue;
        }
    ...
    switch (behaviour) {
        case PathSelectionBehaviour::new_or_least_busy: {
            ...
            std::sort(suitable_paths.begin(), suitable_paths.end(), [](const OnionPath* a, const OnionPath* b) {
            ...
            // Return the path with the fewest pending requests if we had one with no requets, or already have the minimum number of paths for this type
            if (best_path->pending_requests == 0 || candidate_paths.size() >= min_paths_for_type)
                return best_path;

            // Otherwise we want to build a new path (for this PathSelectionBehaviour the assuption is that it'd be faster to build a new path and send the request along that rather than use an existing path)
            return nullptr;
        }
...
void OnionRequestRouter::_send_on_path(OnionPath& path, Request request, network_response_callback_t callback) {
    ...
    std::optional<std::chrono::milliseconds> remaining_overall_timeout = (request.overall_timeout.has_value() ? std::optional{request.time_remaining()} : std::nullopt);
...
void OnionRequestRouter::_decrement_and_cleanup_path(const std::string& path_id, RequestCategory category) {
    ...
    if (auto it = std::find_if(active_paths.begin(), active_paths.end(), [&path_id](const auto& p){ return p.id == path_id; }); it != active_paths.end()) {
    ...
    if (auto it = std::find_if(dying_paths.begin(), dying_paths.end(),  [&path_id](const auto& p){ return p.id == path_id; }); it != dying_paths.end()) {
        if (it->pending_requests > 0)
            it->pending_requests--;
        
        // If this was the last request, we can now safely delete the path
        if (it->pending_requests == 0) {
            log::debug(cat, "[OnionRouter]: Retiring path {} as it has no more pending requests.", path_id);
...
void OnionRequestRouter::_handle_request_failure(
    ...
    if (ed25519PublicKey && ed25519PublicKey->size() == 64 && oxenc::is_hex(*ed25519PublicKey)) {
        try {
            session::network::ed25519_pubkey bad_node_pk = session::network::ed25519_pubkey::from_hex(*ed25519PublicKey);
            auto edpk_view = to_span(bad_node_pk.view());

            auto bad_node_it = std::find_if(path.nodes.begin(), path.nodes.end(), [&edpk_view](const auto& node) { return to_string_view(node.view_remote_key()) == to_string_view(edpk_view); });
            ...
            if (bad_node_it != path.nodes.end()) {
                ...
                if (!replacements.empty()) {
                    log::info(cat, "[OnionRouter Path {}]: Repairing path by replacing node {} with {}.", path.id, bad_node_it->to_string(), replacements[0].to_string());
                    *bad_node_it = replacements[0];
                    path.failure_count = 0;
                    return;
                }

                log::warning(cat, "[OnionRouter Path {}]: Could not find replacement node to repair path.", path.id);
            }

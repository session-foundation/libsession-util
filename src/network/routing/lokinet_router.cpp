...
namespace {
    ...
    oxen::quic::RemoteAddress address_for_destination(const network_destination& dest, const std::string& request_id) {
        std::optional<oxen::quic::RemoteAddress> address;

        std::visit([&address, &request_id](auto&& arg) {
            using T = std::decay_t<decltype(arg)>;

            if constexpr (std::is_same_v<T, oxen::quic::RemoteAddress>) {
                log::trace(cat, "[LokinetRouter Request {}]: Using pre-resolved RemoteAddress.", request_id);
                address = arg;
            } else if constexpr (std::is_same_v<T, service_node>) {
                log::trace(cat, "[LokinetRouter Request {}]: Resolving service_node to RemoteAddress.", request_id);
...
void LokinetRouter::_finish_setup() {
    ...
    log::debug(cat, "[LokinetRouter]: Processing {} requests queued during initialization.", requests_to_process.size());

    for (auto& [address, requests] : requests_to_process) {
        if (!requests.empty()) {
            log::debug(cat, "[LokinetRouter]: Processing {} queued requests for address {}.", requests.size(), address);
...
void LokinetRouter::_send_request_internal(Request request, network_response_callback_t callback) {
    // If the request is being sent to a `ServerDestination` then we need to make a proxied request instead
    if (std::holds_alternative<ServerDestination>(request.destination)) {
        log::critical(cat, "[LokinetRouter Request {}] Server request are currently unsupported!", request.request_id);
        return callback(false, false, -1, {content_type_plain_text},  "Internal error: invalid destination for LokinetRouter");
    }
    ...
    if (!_ready) {
        log::debug(cat, "[LokinetRouter Request {}]: Router not ready, queueing request.", request.request_id);
        ...
        try {
            ...
            log::critical(cat, "[LokinetRouter Request {}]: Dropping after failure to queue due to error: {}.", request.request_id, e.what());
    ...
    // If we should already be establishing a tunnel then we can just add this as a pending request and it'll be picked up once the tunnel is made
    if (_pending_requests.count(address_pubkey_hex)) {
        log::debug(cat, "[LokinetRouter Request {}] Tunnel to {} is pending, queueing request.", request.request_id, address_pubkey_hex);
        _pending_requests[address_pubkey_hex].emplace_back(std::move(request), std::move(callback));
        return;
    }

    // No tunnel exists so we need to start a new one and queue the request
    log::info(cat, "[LokinetRouter Request {}] No tunnel to {}, initiating new tunnel.", request.request_id, address_pubkey_hex);
    std::string initiating_req_id = request.request_id;
    _pending_requests[address_pubkey_hex].emplace_back(std::move(request), std::move(callback));
    _establish_tunnel(address, initiating_req_id);
}

void LokinetRouter::_establish_tunnel(const oxen::quic::RemoteAddress& address, const std::string& initiating_req_id) {
    auto key = address.view_remote_key();
    auto address_pubkey_hex = oxenc::to_hex(key);
    
    if (address_pubkey_hex.size() != 32) {
        log::critical(cat, "[LokinetRouter]: Destination had an invalid remote key, request {} is being dropped.", initiating_req_id);
        ...
        if (auto it = _pending_requests.find(address_pubkey_hex); it != _pending_requests.end()) {
            ...
            log::error(cat, "[LokinetRouter] Failing {} pending requests due to connection failure.", to_fail.size());

            for (auto& [req, cb] : to_fail)
                cb(false, false, -1, {content_type_plain_text}, "Failed to establish tunnel to remote.");
        }
        return;
    }
    ...
    log::debug(cat, "[LokinetRouter Request {}] Establishing new tunnel to {}.", initiating_req_id, address_pubkey_hex);
    lokinet->establish_udp(
            lokinet_address,
            test_port,
            [this, address_pubkey_hex, initiating_req_id](lokinet::tunnel_info info) mutable {
                log::info(cat, "[LokinetRouter Request {}] Tunnel to remote {} established.", initiating_req_id, address_pubkey_hex);
                ...
                if (!requests_to_process.empty()) {
                    log::debug(cat, "[LokinetRouter] Processing {} pending requests on new tunnel to {}.", requests_to_process.size(), info.remote);
            ...
            [this, address_pubkey_hex, initiating_req_id](std::string errmsg) mutable {
                log::info(cat, "[LokinetRouter Request {}] Unable to establish lokinet UDP connection to {} due to error: {}.", initiating_req_id, address_pubkey_hex, errmsg);
                ...
                if (auto it = _pending_requests.find(address_pubkey_hex); it != _pending_requests.end()) {
                    ...
                    log::error(cat, "[LokinetRouter] Failing {} pending requests due to UDP connection failure.", to_fail.size());
...
void LokinetRouter::_send_via_tunnel(lokinet::tunnel_info tunnel, Request request, network_response_callback_t callback) {
    ...
    oxen::quic::RemoteAddress address = address_for_destination(request.destination, request.request_id);
    auto key = address.view_remote_key();

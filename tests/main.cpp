...
std::string g_test_pro_backend_dev_server_url = "";
...
int main(int argc, char* argv[]) {
    ...
    auto cli = session.cli() |
               ...
               Opt(g_test_pro_backend_dev_server_url, "url")["--pro-backend-dev-server-url"](
                       ...
                       "used if compiled with -D TEST_PRO_BACKEND_WITH_DEV_SERVER=1 support. These "
                       "tests are skipped if not specified.");
    ...
    if (g_test_pro_backend_dev_server_url.empty())
        fprintf(stdout, "Skipping --pro-backend-dev-server-url tests, no URL specified.\n");

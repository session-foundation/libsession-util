...
const std::string session_id2 = oxenc::to_hex(sid2);
...
TEST_CASE("Communities 25xxx-blinded pubkey derivation", "[blinding25][pubkey]") {
    ...
    CHECK(to_hex(blind25_id(
                  sid1,
                  "abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"_hex_b)) ==
          "253b991dcbba44cfdb45d5b38880d95cff723309e3ece6fd01415ad5fa1dccc7ac");
    CHECK(to_hex(blind25_id(
                  xpub1,
                  "abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"_hex_b)) ==
...
TEST_CASE("Communities 25xxx-blinded signing", "[blinding25][sign]") {
    constexpr std::array server_pks = {
            "abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"_hex_b,
            "00cdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"_hex_b,
            "abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"_hex_b,
            "999def0123456789abcdef0123456789abcdef0123456789abcdef0123456789"_hex_b,
            "888def0123456789abcdef0123456789abcdef0123456789abcdef0123456789"_hex_b,
            "777def0123456789abcdef0123456789abcdef0123456789abcdef0123456789"_hex_b};
    auto b25_1 = blind25_id(sid1, server_pks[0]);
    auto b25_2 = blind25_id(sid1, server_pks[1]);
    auto b25_3 = blind25_id(sid2, server_pks[2]);
    auto b25_4 = blind25_id(sid2, server_pks[3]);
    auto b25_5 = blind25_id(sid2, server_pks[4]);
    auto b25_6 = blind25_id(sid1, server_pks[5]);

    auto sig1 = blind25_sign(seed1, server_pks[0], "hello"_bytes);
    CHECK(to_hex(sig1) ==
    ...
    CHECK(ed25519::verify(sig1, std::span{b25_1}.last<32>(), "hello"_bytes));

    auto sig2 = blind25_sign(seed1, server_pks[1], "world"_bytes);
    CHECK(to_hex(sig2) ==
    ...
    CHECK(ed25519::verify(sig2, std::span{b25_2}.last<32>(), "world"_bytes));

    auto sig3 = blind25_sign(seed2, server_pks[2], "this"_bytes);
    CHECK(to_hex(sig3) ==
    ...
    CHECK(ed25519::verify(sig3, std::span{b25_3}.last<32>(), "this"_bytes));

    auto sig4 = blind25_sign(seed2, server_pks[3], "is"_bytes);
    CHECK(to_hex(sig4) ==
    ...
    CHECK(ed25519::verify(sig4, std::span{b25_4}.last<32>(), "is"_bytes));

    auto sig5 = blind25_sign(seed2, server_pks[4], ""_bytes);
    CHECK(to_hex(sig5) ==
    ...
    CHECK(ed25519::verify(sig5, std::span{b25_5}.last<32>(), ""_bytes));

    auto sig6 = blind25_sign(seed1, server_pks[5], "omg!"_bytes);
    CHECK(to_hex(sig6) ==
    ...
    CHECK(ed25519::verify(sig6, std::span{b25_6}.last<32>(), "omg!"_bytes));

    // Test that it works when given just the seed instead of the whole sk:
    auto sig6b = blind25_sign(seed1.first<32>(), server_pks[5], "omg!"_bytes);
    CHECK(to_hex(sig6b) ==
          "322e280fbc3547c6b6512dbea4d60563d32acaa2df10d665c40a336c99fc3b8e4b13a7109dfdeadab2ab58b2"
          "cb314eb0510b947f43e5dfb6e0ce5bf1499d240f");
    CHECK(ed25519::verify(sig6b, std::span{b25_6}.last<32>(), "omg!"_bytes));
}
...
TEST_CASE("Communities 15xxx-blinded signing", "[blinding15][sign]") {
    ...
    constexpr std::array server_pks = {
            ...
            "888def0123456789abcdef0123456789abcdef0123456789abcdef0123456789"_hex_b,
            "777def0123456789abcdef0123456789abcdef0123456789abcdef0123456789"_hex_b};
    // Use blind15_key_pair for pubkeys: avoids the sign ambiguity of blind15_id
    auto [b15_1_pk, _1] = blind15_key_pair(seed1, server_pks[0]);
    auto [b15_2_pk, _2] = blind15_key_pair(seed1, server_pks[1]);
    auto [b15_3_pk, _3] = blind15_key_pair(seed2, server_pks[2]);
    auto [b15_4_pk, _4] = blind15_key_pair(seed2, server_pks[3]);
    auto [b15_5_pk, _5] = blind15_key_pair(seed2, server_pks[4]);
    auto [b15_6_pk, _6] = blind15_key_pair(seed1, server_pks[5]);

    auto sig1 = blind15_sign(seed1, server_pks[0], "hello"_bytes);
    CHECK(to_hex(sig1) ==
    ...
    CHECK(ed25519::verify(sig1, b15_1_pk, "hello"_bytes));

    auto sig2 = blind15_sign(seed1, server_pks[1], "world"_bytes);
    CHECK(to_hex(sig2) ==
    ...
    CHECK(ed25519::verify(sig2, b15_2_pk, "world"_bytes));

    auto sig3 = blind15_sign(seed2, server_pks[2], "this"_bytes);
    CHECK(to_hex(sig3) ==
    ...
    CHECK(ed25519::verify(sig3, b15_3_pk, "this"_bytes));

    auto sig4 = blind15_sign(seed2, server_pks[3], "is"_bytes);
    CHECK(to_hex(sig4) ==
    ...
    CHECK(ed25519::verify(sig4, b15_4_pk, "is"_bytes));

    auto sig5 = blind15_sign(seed2, server_pks[4], ""_bytes);
    CHECK(to_hex(sig5) ==
    ...
    CHECK(ed25519::verify(sig5, b15_5_pk, ""_bytes));

    auto sig6 = blind15_sign(seed1, server_pks[5], "omg!"_bytes);
    CHECK(to_hex(sig6) ==
    ...
    CHECK(ed25519::verify(sig6, b15_6_pk, "omg!"_bytes));

    // Test that it works when given just the seed instead of the whole sk:
    auto sig6b = blind15_sign(seed1.first<32>(), server_pks[5], "omg!"_bytes);
    CHECK(to_hex(sig6b) ==
          "946725055399376ecebb605c79f845fbf689a47f98507c2a1f239516fd9c9104e19fe533631c27ba4e744457"
          "4f0e4f0f0d422b7256ed63681a3ab2fe7e040601");
    CHECK(ed25519::verify(sig6b, b15_6_pk, "omg!"_bytes));
}

TEST_CASE("Version 07xxx-blinded pubkey derivation", "[blinding07][key_pair]") {
    REQUIRE(sodium_init() >= 0);

    auto [pubkey, seckey] = blind_version_key_pair(seed1);
...
TEST_CASE("Version 07xxx-blinded signing", "[blinding07][sign]") {
    ...
    auto signature = blind_version_sign(seed1, Platform::desktop, 1234567890);
    ...
    auto [pk, sk] = blind_version_key_pair(seed1);
    auto method = "GET"sv;
    ...
    auto req_sig = blind_version_sign_request(seed1, timestamp, method, path, body);
    CHECK(ed25519::verify(req_sig, pk, full_message));
}

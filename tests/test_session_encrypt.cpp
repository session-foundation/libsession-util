...
TEST_CASE("Session protocol encryption", "[session-protocol][encrypt]") {
    ...
    REQUIRE(oxenc::to_hex(sid_raw) ==
            "05d2ad010eeb72d72e561d9de7bd7b6989af77dcabffa03a5111a6c859ae5c3a72");

    const auto seed2 = "00112233445566778899aabbccddeeff00000000000000000000000000000000"_hex_b;
    auto [ed_pk2, ed_sk2] = ed25519::keypair(seed2);
    auto curve_pk2 = ed25519::pk_to_x25519(ed_pk2);
    REQUIRE(oxenc::to_hex(ed_pk2) ==
            "5ea34e72bb044654a6a23675690ef5ffaaf1656b02f93fb76655f9cbdbe89876");
    REQUIRE(oxenc::to_hex(curve_pk2) ==
            "aa654f00fc39fc69fd0db829410ca38177d7732a8d2f0934ab3872ac56d5aa74");
    auto sid2 = "05" + oxenc::to_hex(curve_pk2);
    REQUIRE(sid2 == "05aa654f00fc39fc69fd0db829410ca38177d7732a8d2f0934ab3872ac56d5aa74");
    std::vector<std::byte> sid_raw2;
    oxenc::from_hex(sid2.begin(), sid2.end(), std::back_inserter(sid_raw2));
    REQUIRE(oxenc::to_hex(sid_raw2) ==
            "05aa654f00fc39fc69fd0db829410ca38177d7732a8d2f0934ab3872ac56d5aa74");

    SECTION("full secret, prefixed sid") {
        auto enc = encrypt_for_recipient(ed_sk, sid_raw2, to_span("hello"));
        CHECK(to_string(enc) != "hello");

        CHECK_THROWS(decrypt_incoming(ed_sk, enc));

        auto [msg, sender] = decrypt_incoming(ed_sk2, enc);
        CHECK(oxenc::to_hex(sender) == oxenc::to_hex(ed_pk));
        CHECK(to_string(msg) == "hello");

        auto broken = enc;
        broken[2] ^= std::byte{0x02};
        CHECK_THROWS(decrypt_incoming(ed_sk2, broken));
    }
    SECTION("only seed, unprefixed sid") {
        ...
        auto enc =
                encrypt_for_recipient(ed25519::extract_seed(ed_sk), sid_raw2, to_span(lorem_ipsum));
...
TEST_CASE("Session protocol deterministic encryption", "[session-protocol][encrypt]") {
    ...
    auto enc1 = encrypt_for_recipient(ed_sk, sid_raw2, to_span("hello"));
    auto enc2 = encrypt_for_recipient(ed_sk, sid_raw2, to_span("hello"));
    REQUIRE(enc1 != enc2);

    auto enc_det = encrypt_for_recipient_deterministic(ed_sk, sid_raw2, to_span("hello"));
    CHECK(enc_det != enc1);
    CHECK(enc_det != enc2);
    ...
    auto [msg, sender] = decrypt_incoming(ed_sk2, enc_det);
    CHECK(oxenc::to_hex(sender) == oxenc::to_hex(ed_pk));
    CHECK(to_string(msg) == "hello");
}

static session::b33 prefixed(std::byte prefix, const session::b32& pubkey) {
    session::b33 result;
    result[0] = prefix;
...
TEST_CASE("Session blinding protocol encryption", "[session-blinding-protocol][encrypt]") {
    ...
    REQUIRE(oxenc::to_hex(sid_raw) ==
            "05d2ad010eeb72d72e561d9de7bd7b6989af77dcabffa03a5111a6c859ae5c3a72");
    auto [blind15_pk, blind15_sk] = blind15_key_pair(ed_sk, server_pk);
    auto [blind25_pk, blind25_sk] = blind25_key_pair(ed_sk, server_pk);
    auto blind15_pk_prefixed = prefixed(std::byte{0x15}, blind15_pk);
    auto blind25_pk_prefixed = prefixed(std::byte{0x25}, blind25_pk);
    ...
    REQUIRE(oxenc::to_hex(sid_raw2) ==
            "05aa654f00fc39fc69fd0db829410ca38177d7732a8d2f0934ab3872ac56d5aa74");
    auto [blind15_pk2, blind15_sk2] = blind15_key_pair(ed_sk2, server_pk);
    auto [blind25_pk2, blind25_sk2] = blind25_key_pair(ed_sk2, server_pk);
    auto blind15_pk2_prefixed = prefixed(std::byte{0x15}, blind15_pk2);
    auto blind25_pk2_prefixed = prefixed(std::byte{0x25}, blind25_pk2);

    SECTION("blind15, full secret, recipient decrypt") {
        auto enc = encrypt_for_blinded_recipient(
                ed_sk, server_pk, blind15_pk2_prefixed, to_span("hello"));
        CHECK(to_string(enc) != "hello");

        auto [msg, sender] = decrypt_from_blinded_recipient(
                ed_sk2, server_pk, blind15_pk_prefixed, blind15_pk2_prefixed, enc);
        CHECK(sender == sid);
        CHECK(to_string(msg) == "hello");

        auto broken = enc;
        broken[23] ^= std::byte{0x80};  // 1 + 5 + 16 = 22 is the start of the nonce
        CHECK_THROWS(decrypt_from_blinded_recipient(
                ed_sk2, server_pk, blind15_pk_prefixed, blind15_pk2_prefixed, broken));
    }
    SECTION("blind15, only seed, sender decrypt") {
        ...
        auto broken = enc;
        broken[463] ^= std::byte{0x80};  // 1 + 445 + 16 = 462 is the start of the nonce
        CHECK_THROWS(decrypt_from_blinded_recipient(
                ed25519::extract_seed(ed_sk),
                server_pk,
                blind15_pk_prefixed,
    ...
    SECTION("blind25, full secret, sender decrypt") {
        auto enc = encrypt_for_blinded_recipient(
                ed_sk, server_pk, blind25_pk2_prefixed, to_span("hello"));
        CHECK(to_string(enc) != "hello");

        auto [msg, sender] = decrypt_from_blinded_recipient(
                ed_sk, server_pk, blind25_pk_prefixed, blind25_pk2_prefixed, enc);
        CHECK(sender == sid);
        CHECK(to_string(msg) == "hello");
    ...
    SECTION("blind25, full secret, recipient decrypt") {
        ...
        auto [msg, sender] = decrypt_from_blinded_recipient(
                ed_sk2, server_pk, blind25_pk_prefixed, blind25_pk2_prefixed, enc);
        CHECK(sender == sid);
        CHECK(to_string(msg) == "hello");
...
TEST_CASE("Session ONS response decryption", "[session-ons][decrypt]") {
    ...
    auto ciphertext =
            ...
            "1580d9a8c9b8a64cacfec97"_hex_b;
    auto ciphertext_legacy =
            "dbd4bc89bd2c9e5322fd9f4cadcaa66a0c38f15d0c927a86cc36e895fe1f3c532a3958d972563f52ca858e94eec22dc360"_hex_b;
    constexpr auto nonce = "00112233445566778899aabbccddeeff00ffeeddccbbaa99"_hex_b;
...
TEST_CASE("Session ONS response decryption C API", "[session-ons][session_decrypt_ons_response]") {
    ...
    auto ciphertext =
            "3575802dd9bfea72672a208840f37ca289ceade5d3ffacabe2d231f109d204329fc33e28c33"
            "1580d9a8c9b8a64cacfec97"_hex_u;
    auto ciphertext_legacy =
            "dbd4bc89bd2c9e5322fd9f4cadcaa66a0c38f15d0c927a86cc36e895fe1f3c532a3958d972563f52ca858e94eec22dc360"_hex_u;
...
TEST_CASE("Session push notification decryption", "[session-notification][decrypt]") {
    ...
    auto payload =
            ...
            "ab4aca5f0991e99eb0344ceeafa"_hex_b;
    auto payload_padded =
            "00112233445566778899aabbccddeeff00ffeeddccbbaa991bcba42892762dbeecbfb1a375f"
            "ab4aca5f0991e99eb0344ceeafa"_hex_b;
...
TEST_CASE("xchacha20", "[session][xchacha20]") {
    ...
    auto payload =
            "da74ac6e96afda1c5a07d5bde1b8b1e1c05be73cb3c84112f31f00369d67154d00ff029090b069b48c3cf603d838d4ef623d54"_hex_b;
    constexpr auto enc_key =
            "0123456789abcdef0123456789abcdeffedcba9876543210fedcba9876543210"_hex_b;
...
TEST_CASE("v2 PFS+PQ message encryption", "[session-protocol][encrypt][v2]") {
    ...
    const auto seed1 = "0123456789abcdef0123456789abcdef00000000000000000000000000000000"_hex_b;
    auto [sender_ed_pk, sender_ed_sk] = ed25519::keypair(seed1);

    // Recipient: long-term session identity from test keypair 2
    const auto seed2 = "00112233445566778899aabbccddeeff00000000000000000000000000000000"_hex_b;
    auto [recip_ed_pk, recip_ed_sk] = ed25519::keypair(seed2);
    auto recip_curve_pk = ed25519::pk_to_x25519(recip_ed_pk);
    auto recip_x25519_sec = ed25519::sk_to_x25519(recip_ed_sk);

    b33 recip_session_id;
    recip_session_id[0] = std::byte{0x05};
    std::copy(recip_curve_pk.begin(), recip_curve_pk.end(), recip_session_id.begin() + 1);

    // Recipient PFS X25519 account key (deterministic)
    const auto pfs_x25519_sec =
            "aabbccddeeff0011223344556677889900112233445566778899aabbccddeeff"_hex_b;
    ...
    const auto pfs_mlkem_seed =
            ...
            "fedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210"_hex_b;
    std::array<std::byte, mlkem768::PUBLICKEYBYTES> pfs_mlkem_pub;
    std::array<std::byte, mlkem768::SECRETKEYBYTES> pfs_mlkem_sec;
    mlkem768::keygen(pfs_mlkem_pub, pfs_mlkem_sec, pfs_mlkem_seed);
    ...
    CHECK(result.sender_session_id[0] == std::byte{0x05});
    CHECK(!result.pro_signature);

    // The recovered sender session ID matches the sender's X25519 pubkey
    auto sender_curve_pk = ed25519::pk_to_x25519(sender_ed_pk);
    CHECK(std::equal(
    ...
    b32 wrong_x25519_sec;
    std::ranges::copy(pfs_x25519_sec, wrong_x25519_sec.begin());
    wrong_x25519_sec[0] ^= std::byte{0xff};
    auto wrong_x25519_pub = x25519::scalarmult_base(wrong_x25519_sec);
    CHECK_THROWS_AS(
            decrypt_incoming_v2(
